...
from src.column_dict import *
from src.treatment_utils import *
...
def excel_to_dataframe_crustacea(file, suborder = 'Decapoda'):
    ...
    #d = db[db['Current 2']==True].drop(columns = l1).rename(columns={a+'1':a[:-1] for a in l2})
    #b = db[db['Current 2']==False].drop(columns = l2).rename(columns={a+'1':a[:-1] for a in l1})
    ...
    # treatment for taxons columns
    taxon_columns = ['kingdom', 'phylum', 'subphylum','class', 'order', 'suborder', 'family',
